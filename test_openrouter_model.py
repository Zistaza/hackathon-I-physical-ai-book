#!/usr/bin/env python3
"""
Test OpenRouter with the correct model name
"""
import os
...
print("Testing assistant creation with OpenRouter model name...")
...
try:
    ...
    # Create the assistant with OpenRouter-compatible model
    assistant = client.beta.assistants.create(
        ...
        model="openai/gpt-4o",  # Use OpenRouter format
        tools=[retrieval_tool]
    )

    print(f"Assistant created successfully!")
    print(f"Assistant type: {type(assistant)}")
    print(f"Assistant object has id: {hasattr(assistant, 'id')}")

    if hasattr(assistant, 'id'):
        print(f"Assistant ID: {assistant.id}")
    else:
        print(f"Assistant content preview: {str(assistant)[:200]}...")
